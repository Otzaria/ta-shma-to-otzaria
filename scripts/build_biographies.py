"""Fetch all rabanan biographies from the Tashma API and build the
release file (biographies.tsb) plus a plain-data hash (biographies.sha256).

Environment:
  TASHMA_API_KEY  - Tashma API key (scope rabanan:read)
  OBF_KEY         - internal build parameter

Output (in ./out):
  biographies.tsb     - the encoded data file consumed by Otzaria
...
def main():
    ...
    mask = hashlib.sha256(os.environ["OBF_KEY"].encode("utf-8")).digest()
    compressed = gzip.compress(canonical, mtime=0)
    packed = bytes(b ^ mask[i % len(mask)] for i, b in enumerate(compressed))

    os.makedirs("out", exist_ok=True)
    with open("out/biographies.tsb", "wb") as f:
        f.write(b"TSB1" + packed)
    with open("out/biographies.sha256", "w") as f:
        f.write(digest + "\n")

    print(f"Entries: {len(entries)}")
    print(f"Plain size: {len(canonical):,} bytes, file size: {len(packed) + 4:,} bytes")
